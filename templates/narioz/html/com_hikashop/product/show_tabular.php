...
?>
		<div class="product-d-category" id="hikashop_product_category_<?php echo $value->category_id ?>">
	...
	<div id="hikashop_product_description_main_mini" class="hikashop_product_description_main_mini"><?php
		if(!empty($this->element->product_description)) {
			...
			if(!function_exists($function))
				$function = 'substr';
			$resume = $function(strip_tags(preg_replace('#<hr *id="system-readmore" */>.*#is','',$this->element->product_description)),0,700);
			if (!empty($this->element->product_description) && strlen($this->element->product_description)>700)
				$resume .= " ...<a href='#hikashop_show_tabular_description'>".JText::_('READ_MORE')."</a>";
			echo JHTML::_('content.prepare',$resume);
		}
	?></div>
<p><?php echo substr((preg_replace('#<hr *id="system-readmore" */>.*#is','',$this->element->product_description)),0,700); ?></p>
</div>
<!-- EO MINI DESCRIPTION -->
<!-- TOP END EXTRA DATA -->
...
<?php if ($this->config->get('show_code')) { ?>
		...
		<span class="sku_wrapper">SKU: <span class="sku"><?php
